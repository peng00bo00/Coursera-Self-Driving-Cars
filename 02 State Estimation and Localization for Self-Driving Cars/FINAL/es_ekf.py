...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    ## f_k is f[k-1] actually
    f_k = imu_f.data[k-1].T
    C_ns = Quaternion(*q_est[k-1]).to_mat()
    p_est[k] = p_est[k-1] + v_est[k-1] * delta_t + 0.5 * delta_t**2 * (C_ns @ f_k -g)
    v_est[k] = v_est[k-1] + delta_t * (C_ns @ f_k -g)
    ...
    F[3:6, 6:9] = -skew_symmetric(C_ns @ f_k) * delta_t
    
    Q = np.eye(6)
    Q[0:3, 0:3] = (var_imu_f**2) * np.eye(3)
    Q[3:6, 3:6] = (var_imu_w**2) * np.eye(3)
    Q *= delta_t**2
    ...
    while i < len(gnss.t):
        if (gnss.t[i] > imu_f.t[k]) or i == len(gnss.t) - 1:
            break
        i += 1
        
    while j < len(lidar.t):
        if (lidar.t[j] > imu_f.t[k]) or j == len(lidar.t) - 1:
            break
        j += 1
    
    if gnss.t[i] < lidar.t[j] and gnss.t[i] - imu_f.t[k] <= 0.01:
        update = True
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(sensor_var = var_gnss, 
                                                                    p_cov_check = p_cov[k], 
                                                                    y_k = gnss.data[i], 
                                                                    p_check = p_est[k], 
                                                                    v_check = v_est[k], 
    ...
    elif lidar.t[j] < gnss.t[i] and lidar.t[j] - imu_f.t[k] <= 0.01:
        update = True
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(sensor_var = var_lidar, 
                                                                    p_cov_check = p_cov[k], 
                                                                    y_k = lidar.data[j], 
                                                                    p_check = p_est[k], 
                                                                    v_check = v_est[k], 
