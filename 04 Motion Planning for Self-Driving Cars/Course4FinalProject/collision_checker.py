    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                dx = np.array([paths[i][0][j] - goal_state[0] for j in range(len(paths[i][0]))])
                dy = np.array([paths[i][1][j] - goal_state[1] for j in range(len(paths[i][1]))])
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            ddx = paths[i][0][-1] - paths[j][0][-1]
                            ddy = paths[i][1][-1] - paths[j][1][-1]
                            dist = np.linalg.norm(np.array([ddx, ddy]))
